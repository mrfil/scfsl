...
files = dir(fullfile(mainfolder,'*VolumeWeighted.csv'));
    ...
    for k = 1:length(files)
        fid = fopen(fullfile(files(k).folder,files(k).name)); 
              ...
              header = {'GlobalEfficiency','MeanClusteringCoeff','MeanStrength'};
              NBStable = array2table(NBSmat,'VariableNames',header);
              nbstablename=strcat("scfsl_nbs_",string(partID),".txt");
              ...
              tableName = strcat(string(partID),"_scfsl_nbs_rois.txt");
