#Creates ROI masks fom AAL116 parcellation (which has previous been transformed to DTI Space)

import os
import nipype.interfaces.fsl as fsl
import csv

parcellation_num = int(116)
...
with open('{}/{}'.format(os.environ['SCRIPTS_DIR'],parcellation_labels_file), 'r') as f:
	for line in range(parcellation_num):
		...
		if line == parcellation_num-1:
			...
			current_line = current_line.split()
		AAL116_Regions_dict[current_line[1]]=int(current_line[0])
		AAL116_Regions_list.append(current_line[1])

#create text file with all ROI masks
with open('masks.txt', 'w') as f:
	for roi in AAL116_Regions_list:
...
for index in range(parcellation_num):           # index goes 1:70
	...
	x = AAL116_Regions_dict[AAL116_Regions_list[index]]
	get_ROI = fsl.maths.Threshold()
	get_ROI.inputs.in_file = 'aal116_atlas_fslstd.nii.gz'
	get_ROI.inputs.thresh = x-0.5
	get_ROI.inputs.args = '-uthr {}'.format(x+0.5)
	get_ROI.inputs.out_file = '{}.nii.gz'.format(AAL116_Regions_list[index])
	get_ROI.run()

	#get volume
	current_ROI_file='{}.nii.gz'.format(AAL116_Regions_list[index])

	get_volume_ROI = fsl.ImageStats(in_file=current_ROI_file, op_string='-V > ROI_volumes.txt')
	get_volume_ROI.run()

	roi_volumes_line=[AAL116_Regions_list[index]]
