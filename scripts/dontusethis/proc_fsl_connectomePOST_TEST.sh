...
source /Users/pcamach2/Downloads/BIDS_Pipeline/StructConFSL_withSUIT/connectome_variables_TEST_POST.cfg

sublist=$(<subjectsPOST.txt)
...
do
  ...
  STUDY_DATDIR=${STUDY_DATA_DIR}/${sub}/ses-02/Analyze/${STUDY_DTIPTH}/
  #location of bedpost directory
  STUDY_BEDPOSTDIR=${RESULTS_DIR}/${sub}/ses-02/DTI.bedpostX/
  #location of freesurfer subject data
  STUDY_FSDIR=${STUDY_SUBJECTS_DIR}/
  #location of connectome output
  STUDY_CONDIR=${RESULTS_DIR}/${sub}/ses-02/${STUDY_CONN_PATH}

#local TEMPORARY data locations for processing                                               
  DATDIR=${DATA_DIR}/${sub}/ses-02/DTI/analyses/
  RESDIR=${DATA_DIR}/${sub}/ses-02/ConnFSL
  FSDIR=${DATA_DIR}
  DATBEDPOSTDIR=${DATA_DIR}/${sub}/ses-02/DTI/analyses.bedpostX/
...
#   /Applications/freesurfer/bin/bbregister --s "$sub" --mov "$DATDIR"nodif_brain.nii.gz --reg "$RESDIR"/diff_2_fs.data $

   # Invert matrix to take FS to DTI Space:
#   /Applications/freesurfer/bin/mri_vol2vol --mov "$DATDIR"nodif_brain.nii.gz --targ "$SUBJECTS_DIR"/"$sub"/mri/"$parce$
...
flirt -cost mutualinfo -dof 6 -in ${DATDIR}nodif_brain.nii.gz -ref ${STUDY_DATA_DIR}/${sub}/ses-02/Analyze/MPRAGE/IMG_brain.nii -omat FSdiff2rage.mat -out FSdiff_in_rage.nii.gz
...
#attempt to replace this script for running in whole script for multiple subjects
SUITS_parc_file=${STUDY_DATA_DIR}/${sub}/ses-02/Analyze/MPRAGE/iw_Lobules-SUIT_u_a_IMG_brain_seg1.nii
...
flirt -cost mutualinfo -dof 6 -in ${DATDIR}nodif_brain.nii.gz -ref ${STUDY_DATA_DIR}/${sub}/ses-02/Analyze/MPRAGE/IMG_brain.nii -omat diff2rage.mat -out diff_in_rage.nii.gz  
...
cd ${RESDIR}
  ...
  source ${SCRIPTS_DIR}/connectome_variables_withSUITS_POST.cfg
